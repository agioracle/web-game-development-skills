...
Use this guide for 2D sprite generation and normalization. This workflow is intentionally anchored around one approved frame and a whole-strip generation pass because frame-by-frame generation drifts too easily.

This workflow is 2D-specific. If the request is for 3D characters, meshes, or materials, return to the main `../SKILL.md` routing.
...
For live asset generation or edits, use the installed `imagegen` skill in this workspace. This workflow defines the game-specific process; `imagegen` handles the API-backed generation or edit execution.

## Script Recipes

The bundled scripts require Pillow (`python3 -m pip install pillow`). Run the commands below from the skill directory (`skills/web-game-development/`).
...
- Detailed workflow: `./sprite-pipeline.md`
- Shared frontend context: `./ui-frontend.md`
